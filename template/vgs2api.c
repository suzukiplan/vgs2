...
static int gclip(unsigned char n,int* sx,int* sy,int* xs,int* ys,int* dx,int* dy)
{
	...
	/* in the screen */
	if((*sx)<0 || _slot[n].xs < (*sx)+(*xs)
	|| (*sy)<0 || _slot[n].ys < (*sy)+(*ys)
	|| (*dx)+(*xs) < 0 || XSIZE <= *dx
	|| (*dy)+(*ys) < 0 || YSIZE <= *dy) {
		return -1;
	}
...
void vgs2_pallet(int c)
{
	...
	for(i=7;i;i--,b-=32) {
		switch(c) {
			...
			case 2: _PAL[i]=b<<16; break;
			case 3: _PAL[i]=(b<<16)|b; break;
			case 4: _PAL[i]=b<<8; break;
			case 5: _PAL[i]=(b<<8)|b; break;
			case 6: _PAL[i]=(b<<16)|(b<<8); break;
			case 7: _PAL[i]=(b<<16)|(b<<8)|b; break;
		}
	}
	make_pallet();
}
...
void vgs2_line(unsigned char* p,int fx,int fy,int tx,int ty,unsigned char c)
{
	...
	if(!idx || !idy) {
		...
		if(0==idy) for(;fx<=tx;fx++) vgs2_pixel(p,fx,fy,c);
		else for(;fy<=ty;fy++) vgs2_pixel(p,fx,fy,c);
		return;
	}
	w = 1;
	ia  = vgs2_abs(idx);
