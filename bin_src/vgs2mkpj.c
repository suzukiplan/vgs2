...
typedef BOOL(WINAPI* SetSecurityDescriptorControlFnPtr)(IN PSECURITY_DESCRIPTOR pSecurityDescriptor, IN SECURITY_DESCRIPTOR_CONTROL ControlBitsOfInterest,
    ...
    __try {
        ...
        if (fDaclPresent && AclInfo.AceCount) {
            for (CurrentAceIndex = 0; CurrentAceIndex < AclInfo.AceCount; CurrentAceIndex++) {
                ...
                // mask.
                //
                if (EqualSid(pUserSID, &(((ACCESS_ALLOWED_ACE*)pTempAce)->SidStart))) continue;
...
int main(int argc, char* argv[])
{
    ...
    if (31 < strlen(argv[1]) || !isalpha(argv[1][0]) || !isalnum_str(argv[1])) {
        puts("ERROR: invalid company name.");
        return 1;
    }

    if (8 < strlen(argv[2]) || !isalpha(argv[2][0]) || !isupper(argv[2][0]) || !isalnum_str(argv[2])) {
    ...
    len = strlen(argv[3]);
#ifdef _WIN32
    if (63 < len || len < 4 || !isalpha(argv[3][0]) || ':' != argv[3][1] || '\\' != argv[3][2] || !isdir_str(&argv[3][3]) || NULL != strstr(argv[3], "\\\\")) {
#else
    if (63 < len) {
#endif
